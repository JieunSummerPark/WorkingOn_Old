...
public class HW08_CountToEE_07_AddMethod_Question {
	...
	private static void MakeATable(int row, int col) {
		...
		if (row<=0 || col<=0) {
			...
			for (int i=0; i<hooray.length; i++) {	// 걍 row랑 col로 하면 안되나? 둘중에 뭐가 더 편할까?
				for (int j=0; j<hooray[i].length; j++) {
					alphabetRow[i] = (char)('A' + i);
					alphabetCol[j] = (char)('A' + j);
					String alRow = Character.toString(alphabetRow[i]);
					String alCol = Character.toString(alphabetCol[j]);	// char 여러개를 string오로 하는거 이렇게 하는거 맞나?
